...
export interface IProps {
  theme: appColorsType;
  item: { value: string, status: boolean, index: number };
  selectMethod: (index: number) => void;
  index: any;
}

const MethodPicker = ({ theme,
  item, selectMethod, index }: IProps) => {
   return (
    ...
    <TouchableOpacity
      ...
      <View key={index} style={styles.selectedViewStyle}>
        {!item.status ?
          ...
          />
          :
          <View  key={index} style={[styles.emptyCheck, { borderColor: theme.secondry }]} />
        }
        <Text
...
const styles = StyleSheet.create({
  ...
  emptyCheck: {
    borderWidth: 3,
    width: Platform.OS === 'ios' ? HEADER_IOS_HEIGHT * 0.2 :
               HEADER_ANDROID_HEIGHT * 0.3,
    height: Platform.OS === 'ios' ? HEADER_IOS_HEIGHT * 0.2 :
               HEADER_ANDROID_HEIGHT * 0.3,
    borderRadius: 50,
    display: 'flex',
