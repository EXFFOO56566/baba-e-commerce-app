...
import {
  I18nManager, Platform, SafeAreaView,
...
import { HEADER_ANDROID_HEIGHT, HEADER_IOS_HEIGHT, HEIGHT, WIDTH } from '../components/config';
...
interface IProps {
  ...
  theme: appColorsType;
  reduxLang: any;
  recentTextArray: string[];
  addSearchValueFun: (Value: string) => void;
  emptySearchListFun: () => void;
}

const searchHeader = (theme: appColorsType,
  navigation: StackNavigationProp<RootStackParamList>,
  reduxLang: any, searchText: string,
  onChangeSearch: (value: string) => void,
  onChangeRecentTextArray: (Value: string) => void,
  recentTextArray: string[], setModalVisible: (value: boolean) => void) => (
  <View style={[styles.searchContainerView, {
    backgroundColor: theme.secondryBackgroundColor
  }]}>

    <TouchableOpacity
      style={styles.paddingRi}
      onPress={() => navigation.pop()}>
      <Ionicons
        style={[styles.horiPadding,
        {
      ...
      <View style={styles.closeIconView}>
        ...
        >
          <FontAwesome
            style={[styles.horiPadding,
            {
              color: searchText.length > 0 ? theme.secondry :
        ...
        <TouchableOpacity onPress={() => setModalVisible(true)}>
          <Ionicons
            ...
            styles.crossIcon,
            {
              ...
              borderColor: searchText.length > 0 ?
...
const recentText = (theme: appColorsType, text: string, index: number,
  onChangeSearch: (value: string) => void) => (
  <TouchableOpacity
    key={index}
    onPress={() => {
...
const tagsFun = (theme: appColorsType, array: {
  name: string,
  isSelected: boolean
}[],
  index: number,
  ...
  <TouchableOpacity
    ...
    style={[styles.textTouchable, {
      backgroundColor: array[index].isSelected ? theme.primary : theme.secondryBackgroundColor
    }]}>
...
const colorsFun = (theme: appColorsType, array: {
  name: string,
  isSelected: boolean
}[],
  index: number,
  ...
  <TouchableOpacity
    ...
    style={[styles.colorsView, {
      ...
      borderColor: array[index].isSelected ? theme.primaryTextColor : theme.primaryBackgroundColor,
    }, {
...
const attributesFun = (theme: appColorsType, array: {
  name: string,
  isSelected: boolean
}[],
  index: number,
  ...
  <TouchableOpacity
    ...
    style={[styles.textTouchable, styles.textAttributes, {
      borderColor: array[index].isSelected ? theme.primaryTextColor : theme.primary,
      backgroundColor: array[index].isSelected ? theme.primary : theme.secondryBackgroundColor
    }]}>
...
const recentSearchLabel = (theme: appColorsType, searchText: string,
  emptySearchListFun: () => void, reduxLang: any) => (
...
const App = ({ navigation, theme, reduxLang,
  recentTextArray, addSearchValueFun, emptySearchListFun }: IProps) => {
  // Header Settings
  useLayoutEffect(() => {
  ...
  const [productsCategoryList, setProductsCategoryList] = useState([
    { name: 'Men', isSelected: false }, { name: 'Women', isSelected: false },
    { name: 'T-Shirts', isSelected: false }, { name: 'Jackets', isSelected: false },
    { name: 'Bags', isSelected: false }, { name: 'Posters', isSelected: false },
    { name: 'Uncategorized', isSelected: false }
  ]);

  const [productsTagsList, setProductsTagsList] = useState([
    { name: '#Green', isSelected: false }, { name: '#red', isSelected: false },
    { name: '#Shirt', isSelected: false }, { name: '#2 kg', isSelected: false }
  ]);

  const [attributes, setAttributes] = useState([
    { name: reduxLang.Colors, isSelected: true }, { name: reduxLang.Size, isSelected: false },
    { name: reduxLang.Weight, isSelected: false }
  ]);

  const [colorsAttr, setColorsAttr] = useState([
    { name: 'red', isSelected: true }, { name: 'green', isSelected: false },
    { name: 'blue', isSelected: false }
  ]);

  const [sizeAttr, setSizeAttr] = useState([
    { name: 'L', isSelected: true }, { name: 'M', isSelected: false },
    { name: 'S', isSelected: false }, { name: 'XL', isSelected: false }
  ]);

  const [weightAttr, setWeightAttr] = useState([
    { name: '2kg', isSelected: true }, { name: '3kg', isSelected: false },
    { name: '5kg', isSelected: false }
  ]);
  const [sliderValue, setSliderValue] = useState(0);


  function sortByText(text: string, isSelected: boolean) {
    return (
      <TouchableOpacity style={[styles.sortByText, {
        borderBottomColor: theme.secondryBackgroundColor
      }]}>
        <Text style={[styles.sortByTextStyle, {
  ...
  return (
    ...
    >
      ...
      <Modal
        ...
        <TouchableWithoutFeedback onPress={() => setModalVisibleSortBy(!modalVisibleSortBy)}>
      ...
      <Modal
        ...
        <View style={[styles.modalView, {
          ...
          <View style={styles.recentSearchArrayView}>
            {
              productsCategoryList.map((value, index) =>
                tagsFun(theme, productsCategoryList, index, setProductsCategoryList))
            }
          </View>
          ...
          <View style={styles.sliderTextView}>
            ...
            <Text style={[styles.sliderTextValue, styles.sliderTextBold,
            {
              color: theme.primary,
              fontSize: theme.appFontSize.mediumSize
            }]}>
          ...
          <View style={styles.filterBtns}>
            ...
            <CustomBtn onPressFun={() => { }} width={WIDTH * 0.3} theme={theme} title={reduxLang.Filter}></CustomBtn>
      ...
      {/* //Search Header */}
      { searchHeader(theme, navigation, reduxLang, searchText, onChangeSearch, addSearchValueFun, recentTextArray, setModalVisible)}

      <View style={styles.selectedFilterText}>
        <Text style={[styles.selectedTextMargin, {
          color: theme.primary,
          fontSize: theme.appFontSize.mediumSize,
          fontFamily: theme.appFontSize.fontFamily
        }]}>
        ...
        <TouchableOpacity style={styles.sortByIconText}
...
const styles = StyleSheet.create({
  ...
  textTouchable: {
    ...
    alignItems: 'center',
    borderRadius: 15,
    padding: 7, margin: 5, paddingTop: 5,
    paddingBottom: 5
  },
  closeIconView: {
    flexDirection: 'row',
...
const mapDispatchToProps = (dispatch: any) => ({
  ...
  addSearchValueFun: (value: string) => dispatch(addSearchValue(value)),
});
